...
import {
  ...
  inventoryPortfolioSections,
} from "@/config/inventoryPortfolio";
...
export default function InventoryCaseStudy({
  ...
  const slug = "inventory-management";
  const intro = localizeIntro(slug, inventoryPortfolioIntro, locale);
  const sections = localizeSections(slug, inventoryPortfolioSections, locale);

  return (
    <article className="page-container max-w-4xl py-10 pb-24 md:py-14 md:pb-28">
      <CaseStudyBack />
      <CaseStudyTags tags={["System Design", "POS", "UX/UI"]} />
